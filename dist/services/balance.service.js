...
class BalanceService {
    ...
    async getUserGroupBalance(userId, groupId) {
        ...
        if (!member) {
            throw new errors_1.AppError("User is not a member of this group", 404);
        }
        const balanceMap = await this.calculateGroupBalances(groupId);
        const owes = [];
        const owedBy = [];
        for (const [key, amount] of Object.entries(balanceMap)) {
            ...
            if (from === userId && amount > 0) {
                const toUser = await prisma_1.default.user.findUnique({ where: { id: to } });
                owes.push({
                    userId: to,
                ...
                const fromUser = await prisma_1.default.user.findUnique({ where: { id: from } });
                owedBy.push({
                    userId: from,
